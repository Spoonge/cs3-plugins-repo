...
private fun getSeasonFromString(sName: String): Int {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        var posterUrl = sdetails.select("img")?.attr("data-src")
        val year = sdetails.select("li:has(.fa-clock) a").text()?.getIntFromText()
        var title = doc.select("h2.postTitle").text().cleanTitle().replace("$year","")
        val isMovie = doc.select(".EmbedCode").isNotEmpty()
            ...
            if(doc.select(".BlockItem a").attr("href").contains("/series/")){//seasons
                doc.select(".BlockItem").map { seas ->
                    ...
                    if(pagEl) {
					    ...
					    element.select(".StarsIMDB strong")?.text().toRatingInt(),
                                            )
                                        )
                                }
                            }
				    ...
				    el.select(".StarsIMDB strong")?.text().toRatingInt(),
                                    )
                               )
                        }
                    }
